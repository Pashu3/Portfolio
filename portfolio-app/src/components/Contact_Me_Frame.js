...
import './Contact_Me_Frame.css';

const ContactMeFrame = ({ isOpen, setIsOpen }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://portfolio-67t0.onrender.com/api/contactme', {
        method: 'POST',
        headers: {
      ...
      {isOpen && (
        <div className={`frame-overlay ${isMaximized ? 'maximized' : ''} ${isMinimized ? 'minimized' : ''}`}>
          <div className="frame-header">
            <div className="frame-title">Contact Me</div>
          ...
          <div className="frame-content">
            {!thankYouMessage ? (
              <form onSubmit={handleFormSubmit}>
                ...
                <div className="sub-heading">
                  Message: <textarea name="message" placeholder="Enter your message" required></textarea>
                </div>
